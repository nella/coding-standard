 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Nella_Sniffs_Classes_ClassDeclarationSniff extends PEAR_Sniffs_Classes_ClassDeclarationSniff
{
		...
		foreach ($classNames as $i => $className) {
			...
			if ($checkingImplements === true && $multiLineImplements === true) {
				...
				if ($tokens[($className - 1)]['code'] === T_COMMA
					...
					$whitespaceBeforClass = $className - 1;
					while ($tokens[$whitespaceBeforClass]['code'] === T_NS_SEPARATOR) {
						$whitespaceBeforClass -= 1;
						if ($tokens[$whitespaceBeforClass]['code'] === T_STRING) {
							$whitespaceBeforClass -= 1;
						}
					}
					if ($tokens[$whitespaceBeforClass]['code'] !== T_WHITESPACE) {
						$spaceBefore = 0;
					} else {
						$spaceBefore = strlen($tokens[$whitespaceBeforClass]['content']);
					}
			...
			if ($tokens[($className + 1)]['code'] !== T_COMMA) {
				if ($i !== ($classCount - 1)) {
					$implementsAfterClass = $className + 1;
					$currentClassId = $i;
					while ($tokens[$implementsAfterClass]['code'] === T_NS_SEPARATOR) {
						$implementsAfterClass += 1;
						if ($tokens[$implementsAfterClass]['code'] === T_STRING) {
							$implementsAfterClass += 1;
							$currentClassId++;
						}
					}
					if ($tokens[$implementsAfterClass]['code'] === T_COMMA || $currentClassId === ($classCount - 1)) {
						continue;
					}

					// This is not the last class name, and the comma
					// is not where we expect it to be.
					if ($tokens[($implementsAfterClass + 1)]['code'] !== T_IMPLEMENTS) {
						$error = 'Expected 0 spaces between "%s" and comma; $%s found';
						$data  = array(
									$tokens[$className]['content'],
									strlen($tokens[$implementsAfterClass]['content']),
								 );
